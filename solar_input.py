...
def parse_star_parameters(line, star):
    ...
    **star** — объект звезды.
    ...
    for attr, val in zip('R color m x y Vx Vy'.split(), 
        line.strip().split()[1:]):
        setattr(star, attr, val if attr=='color' else float(val))
...
def parse_planet_parameters(line, planet):
    ...
    **planet** — объект планеты.
    ...
    for attr, val in zip('R color m x y Vx Vy'.split(), 
        line.strip().split()[1:]):
        setattr(planet, attr, val if attr=='color' else float(val))
    ...
    **space_objects** — список объектов планет и звёзд
    ...
    with open(output_filename, 'a') as out_file:
        for obj in space_objects:
            attribute_string = obj.type + ' '
            for attr in 'R color m x y Vx Vy'.split():
